...
func generateNewChar():
	...
	API_006_Player.setCurrentCharByDict(filledChargenPreset)




func getFilledChargenPreset():
	var dictOfPlayer = API_006_Player.chargenSelecttion
	var kitArray
	if API_006_Player.chargenSelecttion["Kits"] != "NULL":
		var randomKit    = randomSelectkit( API_006_Player.chargenSelecttion["Kits"])
		var arr          = []
		kitArray         = []
		
		if API_006_Player.chargenSelecttion["Kits"] is String :
	...
	API_006_Player.chargenSelecttion["Kits"] = kitArray
...
func insertDebugKits(filledChargenPreset:Dictionary):
	var kits = Util.JSONParser.fileToDictionary("res://config/debugKitsAtStart.json")
...
func fillBySpecificKit(kitArr):
	...
	for i in kitArr.size():
		...
		if kitArr[i] != "NULL":
			var kit = API_001_Atlas.getKitByDrirectName(kitArr[i])
...
func randomSelectkit(kit:String):
	var group = API_001_Atlas.getKitGroup(kit)
