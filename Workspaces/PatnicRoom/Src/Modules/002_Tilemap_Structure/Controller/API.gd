...
func updateDirtyTiles():
	for i in dirtyTiles.size():
		...
		if is_instance_valid(currentNode):
...
func getTilemapLayerByEntity(ent):
	...
	match layer:
		ENUM.SOKRATILES.LAYER.MEAT_GROUND: 			return SokraTiles.getMeatFloor()
		ENUM.SOKRATILES.LAYER.MEAT_INTERACTABLE: 	return SokraTiles.getMeatInteract()
		
		ENUM.SOKRATILES.LAYER.MAGIC_GROUND: 		return SokraTiles.getMagicFloor()
		ENUM.SOKRATILES.LAYER.MAGIC_INTERACTABLE: 	return SokraTiles.getMagicInteract()
		
		ENUM.SOKRATILES.LAYER.MATRIX_GROUND:		return SokraTiles.getMatrixFloor()
		ENUM.SOKRATILES.LAYER.MATRIX_GROUND: 		return SokraTiles.getMatrixInteract()
