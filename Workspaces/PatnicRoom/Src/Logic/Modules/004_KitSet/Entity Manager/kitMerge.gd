...
func getEffect(merge:KitSetEntity):
	merge.effect = API_007_Effect.createEffectByDict( merge.base.effectReference() )
...
func mergeWithKitpart(merge:KitSetEntity,kitpart):
	...
	for i in statsForChange.size():
		...
		var slotToChange = API_004_KitSet.getSlot(merge,"KITSET",int(currentStat["unit"]),currentStat["slot"])
