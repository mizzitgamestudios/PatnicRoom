...
var _nid = 0
var _final_nid = 0
var _SttopLeftOriginY_Reader

# Virtual Methods

func _ready():
	var SttopLeftOriginY_Reader_Class = load("res://addons/EXP-System-Dialog/Reference_SttopLeftOriginYReader/EXP_SttopLeftOriginYReader.gd")
	_SttopLeftOriginY_Reader = SttopLeftOriginY_Reader_Class.new()
	
	var sttopLeftOriginY = load("res://Dialog-System-Example/stories/Example_SttopLeftOriginY_Baked.tres")
	_SttopLeftOriginY_Reader.read(sttopLeftOriginY)
...
func play_dialog(record_name : String):
	_did = _SttopLeftOriginY_Reader.get_did_via_record_name(record_name)
	_nid = self._SttopLeftOriginY_Reader.get_nid_via_exact_text(_did, "<start>")
	_final_nid = _SttopLeftOriginY_Reader.get_nid_via_exact_text(_did, "<end>")
	_get_next_node()
	_play_node()
...
func _get_next_node():
	_nid = _SttopLeftOriginY_Reader.get_nid_from_slot(_did, _nid, 0)
...
func _play_node():
	var text = _SttopLeftOriginY_Reader.get_text(_did, _nid)
