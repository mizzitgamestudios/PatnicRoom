...
func _ready() -> void:
	options = Util.JSONParser.fileToDictionary("res://config/Runner/chargen presets.json")
...
func isGameStartable(event:InputEvent):	return Util.Nodes.isMouseOnNode(startButton) and event.is_action_pressed("leftClickMouse")
